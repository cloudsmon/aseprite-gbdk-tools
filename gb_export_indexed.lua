...
local function gen_tiles_cstr(content_o, bankref_name, metadata_str, include_tile_ex)
	local o = ""
    o = o .. "// AUTOGENERATED by gb_export_indexed.lua\n"
    o = o .. metadata_str
    o = o .. "#pragma bank 255\n#include <stdint.h>\n#include <gbdk/platform.h>\n"
    if include_tile_ex then
        o = o .. "#include \"tile_ex.h\"\n"
    end
    o = o .. "\n"
...
local function gen_tiles_hstr(arr_prefix, bankref_name, nr_tiles, size_map, num_tilesets)
	local o = ""
    o = o .. "// AUTOGENERATED by gb_export_indexed.lua\n"
    ...
    if num_tilesets > 0 then
        if num_tilesets == 1 then 
            o = o .. string.format("extern const tilesubset_t %s_tileset;\n", arr_prefix)
        elseif num_tilesets > 1 then
            o = o .. string.format("extern const tilesubset_t %s_tileset[%d];\n", arr_prefix, num_tilesets)
        end
    end
    if size_map > 0 then 
        o = o .. string.format("extern const uint8_t %s_map[%d];\n", arr_prefix, size_map)
        o = o .. string.format("extern const tilesubmap_t %s_submap;\n", arr_prefix)
    end
    o = o .."#endif\n"
	return o
end
...
function export_gameboy_tiles()
    ...
    local map_o = ""
    local tileset_o = ""
    local submap_o = "" -- if more than one "sprite" in tilemap
    ...
    local export_tileset = dialog.data["opt_tileset"]
    ...
    if export_tileset == nil then 
        export_tileset = false
    end
    ...
    if not spr_cutoff_cnt then 
        ...
        spr_cutoff_cnt = math.floor(dialog.data["tilecutoff"] )
    end
    ...
    col = 0
        ...
        do
            ...
            map_o = map_o .. string.format("// spr: %u tx: %u, ty: %u\n%s\n", spritecount, col, row, arr_to_bytes_str(spr_table.map))

            spritecount = spritecount + 1
            spr_table_all[spritecount] = spr_table
    ...
    local submap_str = string.format("%s_submap", array_prefix)
    tiles_o = string.format("const uint8_t %s[] = {\n%s\n};\n\n", tile_str, tiles_o)

    local num_tilesets = 0
    if export_tileset then
        local tileset_str = string.format("%s_tileset", array_prefix)

        if spritecount == 1 then
            local n_tiles = #spr_table_all[1].tiles
            tileset_o = string.format("%d, %s", n_tiles, tile_str)
            tiles_o = tiles_o .. string.format("const tilesubset_t %s = {%s};\n\n", tileset_str, tileset_o)
        else
            local tileset_offset = 0
            for i = 1, spritecount do
                local n_tiles = #spr_table_all[i].tiles
                tileset_o = tileset_o .. string.format("{%d, %s + (%d<<4),},\n", n_tiles, tile_str, tileset_offset)
                tileset_offset = tileset_offset + n_tiles
            end
            tiles_o = tiles_o .. string.format("const tilesubset_t %s[%d] = {%s};\n\n", tileset_str, spritecount, tileset_o)

        end
        
        num_tilesets = spritecount
    ...
    if export_map then 
        ...
        map_size = #spr_table.map * spritecount
        -- submap
        local submap_offset = 0
        for i = 1, spritecount do
            submap_o = submap_o .. string.format("%s + %d,\n", map_str, submap_offset)
            submap_offset = submap_offset + #spr_table_all[i].map
        end
        -- assume all submaps have same w x h
        local tw = (size[1] // TILE_SIZE)
        local th = (size[2] // TILE_SIZE)
        submap_o = string.format("%d, %d, {\n%s\n}", tw, th, submap_o)
        tiles_o = tiles_o .. string.format("const tilesubmap_t %s = {\n%s\n};\n\n", submap_str, submap_o)
    end


    local include_tile_ex = false
    if export_map or export_tileset then
        include_tile_ex = true
    end
    local tile_data_c = gen_tiles_cstr(tiles_o, bankref_name, metadata_str, include_tile_ex)
    local tile_data_h = gen_tiles_hstr(array_prefix, bankref_name, tilecnt, map_size, num_tilesets, no_gbs_include)
...
dialog:check{id="opt_tileset", label="Export Tileset"}
